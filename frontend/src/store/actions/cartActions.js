...
export const getCartItems = (token) => {
  return (dispatch) => {
    axios.get(`/cart`, { headers: getConfig(token).header }).then((res) => {
      dispatch({
        type: "GET_CART_ITEMS",
        payload: res.data,
      });
...
export const addToCart = (product, token) => {
  return (dispatch) => {
    axios
      .post(`/cart/add`, product, { headers: getConfig(token).header })
      .then((res) => {
        dispatch({
...
export const updateCart = (product, token) => {
  return (dispatch) => {
    axios
      .put(`/cart/update`, { product }, { headers: getConfig(token).header })
      .then((res) => {
        dispatch({
...
export const deleteFromCart = (product, token) => {
  ...
  return (dispatch) => {
    axios
      .delete(`/cart/delete`, { headers: headers, data: { product } })
      .then((res) => {
        dispatch({
      ...
      .catch((err) => {
        setError(
          ...
          err.response.data.message,
          err.response.status,
          "DELETE_FROM_CART_ITEMS_FAILURE"
        );
      });
  };
};
